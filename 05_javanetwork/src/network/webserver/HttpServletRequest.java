...
public class HttpServletRequest {
	...
	private void init() throws Exception {
		...
		// POST 방식 처리하기
		// 파라미터가 시작라인이 아닌 바디에 넘어오기 때문에 헤더정보를 읽고 
		// 바디에 있는 데이터를 꺼낸다.
		...
		if("POST".equals(method)) {
			...
			while (br.ready()) {	//jdk버그 while(true) 는 블럭에 빠진다! 그래서 대처방법인 ready()조건이
				ch = br.read();
				bodyParam += (char)ch;
			}
	...
	private void setParamMap() throws Exception {
		...
		for (String param : params) {
			...
			paramMap.put(value[0], URLDecoder.decode(value[1], "utf-8"));
